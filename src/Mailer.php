...
class Mailer extends BaseMailer
{
    ...
    public function sendMessage($message)
    {
        try {
            ...
            $mailMessage = new \SendGrid\Mail\Mail();

            $mailMessage->setSubject(
                $message->getSubject()
            );

            $mailMessage->addContents($payload->getContents());
            ...
            if (is_array($message->getTo())) {
                $mailMessage->addTo(
                    key($message->getTo()),
                    current($message->getTo())
                );
            }

            $response = $this->getSendGrid()->send($mailMessage);
